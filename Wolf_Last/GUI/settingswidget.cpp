...
SettingsWidget::SettingsWidget(QRect& rect, Settings *settings, QWidget *parent)
    ...
    , m_settings(settings)
{
   ...
   /* int AREA_WIDTH; - от 10 до 200
    ...
    m_erode->setChecked(m_settings->ERODE);
    m_dilate = new QCheckBox;
    m_dilate->setText("Dilate");
    m_dilate->setChecked(m_settings->DILATE);
    m_proSettingsMode  = new QCheckBox;
    m_proSettingsMode->setText("Pro Settings Mode");
    m_proSettingsMode->setChecked(m_settings->PRO_SETTINGS_MODE);
    ...
    QObject::connect(m_areaWidth, SIGNAL(valueChanged(int)), m_settings, SLOT(changeAreaWidth(int)));
    QObject::connect(m_areaHeight,SIGNAL(valueChanged(int)),m_settings,SLOT(changeAreaHeight(int)));
    QObject::connect(m_shiftFromBorder,SIGNAL(valueChanged(int)),m_settings,SLOT(changeShiftFromBorder(int)));
    QObject::connect(m_maxDeviation,SIGNAL(valueChanged(int)),m_settings,SLOT(changeMaxDeviation(int)));
    QObject::connect(m_coverRatio,SIGNAL(valueChanged(int)),m_settings,SLOT(changeCoverRatio(int)));
    QObject::connect(m_kernelEroSize,SIGNAL(valueChanged(int)),m_settings,SLOT(changeKernelEroSize(int)));
    QObject::connect(m_kernelDilSize,SIGNAL(valueChanged(int)),m_settings,SLOT(changeKernelDilSize(int)));
    QObject::connect(m_threshold,SIGNAL(valueChanged(int)),m_settings,SLOT(changeThreshold(int)));
    QObject::connect(m_delay,SIGNAL(valueChanged(int)),m_settings,SLOT(changeDelay(int)));


    QObject::connect(m_erode, SIGNAL(toggled(bool)), m_settings, SLOT(changeErode(bool)));
    QObject::connect(m_dilate,SIGNAL(toggled(bool)),m_settings,SLOT(changeDilate(bool)));
    QObject::connect(m_proSettingsMode,SIGNAL(toggled(bool)),m_settings,SLOT(changeProSettigsMode(bool)));
